...
class QwenImageClient:
    ...
    async def generate_one(self, prompt: str, *, size: str | None = None) -> bytes:
        ...
        task_id = await self._submit_task(prompt, size=size or settings.QWEN_IMAGE_SIZE)
    ...
    async def _submit_task(self, prompt: str, *, size: str) -> str:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            response = await client.post(settings.QWEN_IMAGE_API_URL, headers=self.headers, json=payload)
        if response.status_code >= 400:
            raise QwenImageGenerationError(f"图片生成任务提交失败：HTTP {response.status_code} {response.text[:200]}")
    ...
    async def _poll_task(self, task_id: str) -> str:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            while time.monotonic() < deadline:
                ...
                if response.status_code >= 400:
                    raise QwenImageGenerationError(f"图片生成任务查询失败：HTTP {response.status_code}")
                ...
                if status in {"FAILED", "CANCELED", "UNKNOWN"}:
                    message = output.get("message") or output.get("task_metrics") or "未知错误"
    ...
    async def _download_image(self, image_url: str) -> bytes:
        ...
        if response.status_code >= 400:
            raise QwenImageGenerationError(f"生成图片下载失败：HTTP {response.status_code}")
