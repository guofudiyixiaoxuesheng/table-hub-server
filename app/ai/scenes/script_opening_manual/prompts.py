...
TIMELINE_SYSTEM_PROMPT = (
    ...
    "若事实锚点含 actStructure（由多个角色本共同标题聚合），它是分幕数量和顺序的最高证据；"
    "必须逐幕生成节点，并把该幕已识别的任务提示转化为玩家动作或 DM 提醒。"
...
def build_timeline_prompt(
    *,
    ...
    return (
        ...
        "}\n\n"
        "要求：\n"
        "1. 若全局事实锚点 JSON 中有 actStructure：以其中 act 的自然顺序确定一共几幕，"
        "每一幕至少对应一个时间线节点；isShared 为 true 的幕次视为共同流程，不能省略或另编幕次。\n"
        "2. 每幕 taskHints 是从角色本摘取的原始任务线索；应将其整理为玩家动作、DM 发放/提醒或控场要点，"
        "不可把未覆盖所有角色的任务误写成全员公共任务。\n"
        "3. 时间线必须按真实开本顺序排列，优先识别“开本前、开场、分幕、私聊、机制、投票、复盘、结算”。\n"
        "4. 每个节点要让新手 DM 看完就知道下一步怎么做，禁止只写抽象总结。\n"
        "5. 控制 JSON 长度：最多 12 个节点；dmAction 不超过 100 字，playerAction 不超过 80 字；"
        "materials 和 riskNotes 各最多 3 项，source 只写一个来源。\n"
        "6. 不确定的幕次、物料、话术、结局，必须写“需人工确认”，不要猜。\n"
        "7. 如果资料只有玩家本，没有 DM 手册，就明确标注风险。\n"
        "8. 输出必须是合法 JSON。"
    )
