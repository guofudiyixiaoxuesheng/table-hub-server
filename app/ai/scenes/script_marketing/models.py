...
class ScriptMarketingAsset(Base):
    ...
    __table_args__ = (
        Index("ix_script_marketing_assets_document_status", "document_id", "status", "created_at"),
    ...
    version_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("knowledge_versions.id", ondelete="SET NULL"), nullable=True)
    version_no: Mapped[int] = mapped_column(Integer, nullable=False, default=1, server_default="1")
    purpose: Mapped[str] = mapped_column(String(40), nullable=False, server_default="script_profile")
    tone: Mapped[str] = mapped_column(String(80), nullable=False, server_default="新手友好、商业宣传")
    manager_feedback: Mapped[str | None] = mapped_column(Text, nullable=True)
    title: Mapped[str] = mapped_column(String(200), nullable=False)
    summary: Mapped[str] = mapped_column(Text, nullable=False)
    selling_points: Mapped[list[str]] = mapped_column(JSONB, nullable=False, default=list, server_default="[]")
    suitable_players: Mapped[list[str]] = mapped_column(JSONB, nullable=False, default=list, server_default="[]")
    ...
    detail_image_prompts: Mapped[list[str]] = mapped_column(JSONB, nullable=False, default=list, server_default="[]")
    detail_image_keys: Mapped[list[str]] = mapped_column(JSONB, nullable=False, default=list, server_default="[]")
    ...
    risk_notes: Mapped[list[str]] = mapped_column(JSONB, nullable=False, default=list, server_default="[]")
    sources: Mapped[list[str]] = mapped_column(JSONB, nullable=False, default=list, server_default="[]")
